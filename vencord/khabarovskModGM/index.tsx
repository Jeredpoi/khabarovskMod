...
import { openNewsComposer } from "./news";
...
const channelContextPatch: NavContextMenuPatchCallback = children => {
    children.push(
        <Menu.MenuItem
            id="khab-news-post"
            label="Опубликовать в новости"
            action={() => openNewsComposer(send)}
        />
    );
};
...
function registerApproveCopy() {
    approveHandler = (event: MouseEvent) => {
        try {
            ...
            console.error("[khabarovskModGM] approve copy failed", error);
        }
    };
...
export default definePlugin({
    ...
    description:
        "Версия для главных модераторов сервера Хабаровск (BlackRussia): выдача наказаний, " +
        "формы, история нарушений и публикация объявлений в канал новостей.",
